	...
	else
	{
		...
		if (commonType.getCategory() == Type::Category::INTEGER)
		{
			if (Token::isCompareOp(op))
				cleanupNeeded = true;
			else if (op == Token::DIV || op == Token::MOD)
				cleanupNeeded = dynamic_cast<IntegerType const&>(commonType).isSigned();
		}
...
void ExpressionCompiler::appendTypeConversion(Type const& _typeOnStack, Type const& _targetType, bool _cleanupNeeded)
{
	...
	{
		if (asserts(_targetType.getCategory() == Type::Category::INTEGER))
			BOOST_THROW_EXCEPTION(InternalCompilerError() << errinfo_comment("Conversion from integer to non-integer type requested."));
		appendHighBitsCleanup(dynamic_cast<IntegerType const&>(_typeOnStack));
	}
	else if (_typeOnStack != _targetType)
	{
		// All other types should not be convertible to non-equal types.
		BOOST_THROW_EXCEPTION(InternalCompilerError() << errinfo_comment("Illegal type conversion requested."));
	}
}
...
void ExpressionCompiler::appendHighBitsCleanup(IntegerType const& _typeOnStack)
{
	if (_typeOnStack.getNumBits() == 256)
		...
		m_context << u256(_typeOnStack.getNumBits() / 8) << eth::Instruction::SIGNEXTEND;
	else
